...
model = tf.keras.Sequential([
        ...
        layers.Conv2D(64, kernel_size=15, padding='valid',strides=1, activation='relu'),
        layers.Conv2D(64, kernel_size=9, padding='valid',strides=1, activation='relu'),
        layers.MaxPool2D(2),
        ...
        layers.Conv3DTranspose(128, kernel_size=(3,3,6), padding='valid',strides=1, activation='relu'),
        layers.Conv3DTranspose(128, kernel_size=(3,3,6), padding='valid',strides=1, activation='relu'),
        layers.BatchNormalization(),
        layers.Conv3DTranspose(64, kernel_size=(3,3,6), padding='valid',strides=1, activation='relu'),
        layers.Conv3DTranspose(64, kernel_size=(3,3,6), padding='valid',strides=1, activation='relu'),
        layers.Conv3DTranspose(64, kernel_size=(3,3,6), padding='valid',strides=1, activation='relu'),
        layers.BatchNormalization(),
...
def treshold(restriction):
  ...
  with open(file_path, 'a') as file:
      ...
      for i in range(n):
          ...
          plt.imshow(X_test[k,:,:,i],cmap='gray')

      ax = fig.add_subplot(3, 3, 2*3 + 1, projection='3d')
      ax.voxels(test_pred_tresholded[k,:,:,:], edgecolor='k')

      ax = fig.add_subplot(3, 3, 2*3 + 2, projection='3d')
