...
model = tf.keras.Sequential([
        ...
        TimeDistributed(Conv2D(32, kernel_size=15, padding='valid',strides=1, activation='relu')),
        TimeDistributed(Conv2D(32, kernel_size=9, padding='valid', strides=1, activation='relu')),
        TimeDistributed(layers.MaxPool2D(2)),
        TimeDistributed(layers.BatchNormalization()),
        TimeDistributed(layers.Conv2D(64, kernel_size=7, padding='valid',strides=1, activation='relu')),
        TimeDistributed(layers.Conv2D(64, kernel_size=7, padding='valid', strides=1, activation='relu')),
        TimeDistributed(layers.MaxPool2D(2)),
        TimeDistributed(layers.BatchNormalization()),
        TimeDistributed(layers.Conv2D(128, kernel_size=3, padding='valid', strides=1, activation='relu')),
        TimeDistributed(layers.Conv2D(128, kernel_size=3, padding='valid', strides=1, activation='relu')),
        TimeDistributed(layers.MaxPool2D(2)),
        TimeDistributed(layers.BatchNormalization()),
        TimeDistributed(layers.Conv2D(256, kernel_size=3, padding='valid', strides=1, activation = 'relu')),
        TimeDistributed(layers.Conv2D(256, kernel_size=3, padding='valid', strides=1, activation = 'sigmoid')),
        TimeDistributed(layers.BatchNormalization()),

        layers.Reshape((6,20, 20, 256)),

        layers.Conv3DTranspose(128, kernel_size=(7,3,3), padding='valid',strides=1, activation='relu'),
        layers.Conv3DTranspose(128, kernel_size=(7,3,3), padding='valid',strides=1, activation='relu'),
        layers.BatchNormalization(),
        layers.Conv3DTranspose(64, kernel_size=(8,5,5), padding='valid',strides=1, activation='relu'),
        layers.Conv3DTranspose(64, kernel_size=(8,5,5), padding='valid',strides=1, activation='relu'),
        layers.BatchNormalization(),
