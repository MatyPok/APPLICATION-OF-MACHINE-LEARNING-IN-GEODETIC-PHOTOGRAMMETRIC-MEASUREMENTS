...
model = tf.keras.Sequential([
        ...
        layers.Conv2D(32, kernel_size=5, padding='same',strides=2, 	activation='relu'),
        layers.Conv2D(64, kernel_size=5, padding='same', strides=2, 	activation='relu'),
        layers.BatchNormalization(),
        layers.Conv2D(128, kernel_size=3, padding='same', strides=2, 		activation='relu'),
        layers.BatchNormalization(),
        layers.Conv2D(32, kernel_size=3, padding='same', strides=1, activation = 		'sigmoid'),

        layers.Reshape((32, 32, 32, 1)),

        layers.Conv3DTranspose(32, kernel_size=3, padding='same',strides=1, 		activation='relu'),
        layers.BatchNormalization(),
        layers.Conv3DTranspose(16, kernel_size=3, padding='same',strides=1, 		activation='relu'),
        layers.BatchNormalization(),
        layers.Conv3D(1, kernel_size=1, padding='same', activation='sigmoid'),
        layers.Reshape((32, 32, 32))
])
# we are using Adam optimizer and Binary Cross entropy loss function and main metric
...
def treshold(restriction):
  ...
  for i in range(n):
      ...
      ax = fig.add_subplot(4, 4, i + n + 1, projection='3d')
      ax.voxels(y_test[i+m,:,:,:], edgecolor='k')

      ax = fig.add_subplot(4, 4, i + n + n + 1, projection='3d')
      ax.voxels(test_pred_tresholded[n+i+m,:,:,:], edgecolor='k')

      ax = fig.add_subplot(4, 4, i + n + n + n + 1, projection='3d')
      ax.voxels(y_test[n+i+m,:,:,:], edgecolor='k')
